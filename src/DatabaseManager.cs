...
namespace SpaceShooter
{
    public static class DatabaseManager
    {
        ...
        {
            createGameTable();
            string insertSQL = "Insert into Game(skor, dalga, oyunSüresi) values (@score, @wave, @gameDuration)";

            using (SQLiteCommand command = new SQLiteCommand(insertSQL))
            {
                command.Parameters.AddWithValue("skor", score);
                command.Parameters.AddWithValue("Seviye", wave);
                command.Parameters.AddWithValue("oyunSüresi", gameDuration);
